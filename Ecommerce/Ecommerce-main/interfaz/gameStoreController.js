...
const GameStoreController = {
  ...
  addToCart: (req, res) => {
    ...
    GameStoreServices.addToCart(userId, productId, quantity, (err, cartItemId) => {
      if (err) return res.status(500).send("Error al añadir el producto al carrito");
  ...
  getOrderDetails: (req, res) => {
    ...
    GameStoreServices.getOrderDetails(orderId, (err, orderDetails) => {
      if (err) return res.status(500).send("Error al obtener los detalles de la orden");
  ...
  createCategory: (req, res) => {
    ...
    GameStoreServices.createCategory({ name, description, image }, (err, newCategory) => {
      if (err) {
